...
def create_storage_condition(db: Session, storage_condition: schemas.StorageConditionCreate):
    db_storage_condition = models.StorageCondition(
        **storage_condition.model_dump())
...
def update_storage_condition(db: Session, condition_id: int, storage_condition: schemas.StorageConditionCreate):
    db_condition = db.query(models.StorageCondition).filter(
        models.StorageCondition.id == condition_id).first()
    if db_condition:
...
def update_warehouse_section(db: Session, section_id: int, warehouse_section: schemas.WarehouseSectionCreate):
    db_section = db.query(models.WarehouseSection).filter(
        models.WarehouseSection.id == section_id).first()
    if db_section:
...
def update_batch(db: Session, batch_id: int, batch: schemas.BatchCreate):
    db_batch = db.query(models.Batch).filter(
        models.Batch.id == batch_id).first()
    if db_batch:
...
def update_qr_code(db: Session, qr_id: int, qr_code: schemas.QRCodeCreate):
    db_qr_code = db.query(models.QRCode).filter(
        models.QRCode.id == qr_id).first()
    if db_qr_code:
